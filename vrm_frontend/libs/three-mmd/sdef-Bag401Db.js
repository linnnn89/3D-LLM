...
//#region src/materials/core/sdef.ts
const sdefDeclaration = `
#if MMD_USE_SDEF
#ifdef USE_SKINNING
...
#endif
`;
const assertAndReplace = (source, expected, replacement, label) => {
	if (!source.includes(expected)) throw new Error(`MMD material shader patch failed: missing ${label} (Three r${REVISION}).`);
	return source.replace(expected, replacement);
};
/** Applies material-local SDEF hooks without mutating Three's global shader chunks. */
const installSdefPatch = (material) => {
	material.defines = {
		...material.defines,
		MMD_USE_SDEF: 1
	};
	...
	material.onBeforeCompile = (shader, renderer) => {
		...
		if (shader.vertexShader.includes("#include <skinnormal_vertex>")) shader.vertexShader = assertAndReplace(shader.vertexShader, "#include <skinnormal_vertex>", `#if MMD_USE_SDEF\n${skinNormalVertex}\n#else\n#include <skinnormal_vertex>\n#endif`, "skinnormal_vertex");
