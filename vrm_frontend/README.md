...
>
> ⚠️ 但**只有少数材质发白**时不是光照问题，见 **3.4**。
> 喜多郁代真正的原因是 3.4，光照只是叠加因素——所以此前单独调低光照并不能解决它。
...
### 3.4 黑色占位法线贴图会让面部整体过曝（喜多郁代的真正病根）

**这是最容易误判成「光照太强」的一个坑**，因为它表现得就像打光过曝：脸和皮肤被洗成一片白、五官消失。

现象上有两个可用于区分的特征：

- 只有**少数材质**发白。光照过强是全局现象，头发/衣服会一起变浅；而这里头发、衣服的颜色完全正常，唯独皮肤、眼睛、身体发白。
- 调低光照**几乎无效**——因为过曝不是灯光加出来的。

**成因**：three.js 的法线贴图计算只把 `normalScale` 的 **xy** 清零，z 仍取贴图蓝通道 `* 2 - 1`：

```glsl
vec3 mapN = texture2D( normalMap, vNormalMapUv ).xyz * 2.0 - 1.0;
mapN.xy *= normalScale;        // 只影响 xy
normal = normalize( tbn * mapN );
```

部分 VRM（如 `喜多郁代.vrm`）用 `_BumpScale = 0` 表示「不要凹凸」，却把 `_BumpMap` 指向一张**纯黑占位图**（`Shader_NoneBlack.png`，8×8，蓝通道 ≈ 0）。此时：

- `normalScale = (0, 0)` → `mapN = (0, 0, -1)` → **法线被整体翻转成 -N**；
- MToon 的 rim 项随即使 `saturate(1 - dot(viewDir, normal))` 饱和到 1，`pow(1, 100)` 仍是 1；
- 于是纯白的 `parametricRimColorFactor` 被**叠加到整个表面**，脸、身体直接顶到 255，眼睛的颜色也被冲淡。

> 对照：同样是 `scale = 0`，如果指向的是**合法法线图**（蓝通道 ≈ 255，如 `由比滨结衣` 的头发），`mapN` 会归一化成 `(0, 0, +1)` = **+N**，完全无害。**判据是蓝通道，不是 scale。**

**修复**（`app.js` 的 `dropDisabledNormalMaps()`，在模型加载后调用）：`normalScale` 为 0 本意就是「不要凹凸」，直接摘掉这张贴图即可。对合法法线图也完全等价（结果都是 +N），故无需区分、可无条件处理。

**不要**试图靠"把光照调暗"绕过这个问题：它只会让正常的头发和衣服一起变暗，而脸依然是白的。

### 3.5 各角色世界尺寸不同，必须归一化「头骨」高度

相机只有一套固定参数（`app.js` 里 `camera.position.set(0, 1.36, 1.25)`、`controls.target.set(0, 1.30, 0)`、FOV 30），
但**各 VRM 的世界尺寸并不一致**。实测 head 骨（头骨枢轴）的世界高度：

| 角色 | head 骨 Y | 相对由比滨 | 缩放系数 |
| --- | --- | --- | --- |
| 由比滨结衣 | 1.4111 | 1.000 | ×1.0000（基准，逐像素不变） |
| 喜多郁代 | 1.6014 | 1.135 | ×0.8811 |
| 雷电将军 | 1.5299 | 1.084 | ×0.9224 |

不处理的话，固定相机框的是**世界坐标**，个子高的角色头部就跑到框外 ——
表现为「只看得见嘴和下巴，脑袋没了」。

**修法**：`app.js` 的 `normalizeModelScale()`（模型加载后立即调用）把 `vrm.scene`
整体缩放到统一的头骨高度，基准取由比滨结衣的 `1.4111`。选她当基准的理由是
**她原本表现正常，归一化后必须逐像素不变**。

> ⚠️ **不要按「包围盒高度」归一。** 喜多郁代的呆毛把包围盒顶到 1.9395，而她的 head 骨在
> 1.6014 —— 用包围盒会被头发带偏，把角色整体缩得过小、脸部构图偏低。
> **头骨才是取景真正要锚的地方。**

归一之后，相机、取景档位、`OrbitControls` 的 `minDistance`/`maxDistance`、点击 raycast
全都不需要再按角色区分。

**验证方法**：控制台会打印 `[VRM] 尺寸归一化：head 骨 X → ×Y`。把所有角色切一遍，
每行都应满足 `head 骨 × 缩放系数 ≈ 1.4111`；截图里三个角色的**眼睛都落在画面自上而下约 1/3 处**。

> 遗留：喜多郁代的发量/呆毛相对头骨更高，归一化后她的**发梢**会比由比滨更贴近画面上缘
> （约 4% 被切）。这是按头骨对齐的必然结果 —— 换来的是脸部构图一致。若更想要"头发也完整"，
> 需要改成按包围盒留白，但那样脸会整体偏低。
...
- **喜多（VRM 0.x）**：贴图全部内嵌（10 张），无外部依赖。原始骨骼为 Blender 命名风格（`hips`/`upper_arm.R`）。此前"白模、没有五官"主要是 **3.4 的黑色占位法线贴图**所致（皮肤/眼睛/身体三个材质），色调映射与光照强度是次要因素，**模型本身没有问题**。
- **爱蜜莉雅**：模型缺失（此前误放了衣服文件），已归档到 `characters/_archived/zh_爱蜜莉雅.yaml`，前端引用已移除。
- **依蕾娜**：原为未优化的高模（106 万顶点 / 105 万面，56 MB），曾用 [`scripts/optimize_vrm.py`](../scripts/optimize_vrm.py) 减面到 16.4 万面，但减面后的外观始终无法修好，**该角色已整体移除**：人设归档到 `characters/_archived/zh_依蕾娜.yaml`，模型移入 `trash/`，`CHARACTER_META`（含 `依蕾娜` / `伊蕾娜` 两个别名键）、`VOICE_LINES` 与角色下拉的引用已一并清除。
  > ⚠️ `vrm-models/` 是 gitignore 的，模型文件不在版本控制里，**删除即不可从 git 恢复**，所以这里是移入 `trash/` 而非直接删（`trash/` 也已加入 `.gitignore`）。
...
- **自检通过 ≠ 能加载**。脚本的自检覆盖骨骼索引、纹理索引、顶点属性及其编码，但真正的渲染行为只有引擎能验证。实测一个高模 VRM 前两次优化都通过了自检却仍有问题：第一次"骨骼映射 54/54 一致"但加载即报错（贴图被回收），第二次属性齐全却**渲染全黑**（量化改写了 UV）。替换前请务必实际打开页面确认。
...
- 全局比例简化会**无差别地砍掉小网格**：实测某模型的 `eyes` 网格在 `-si 0.1` 下从 360 面掉到 28 面。这类面部细节建议配合 `-se` 使用，缩小比例、靠误差约束保护。
- 回退：用优化前备份的原模型覆盖 `vrm-models/<角色>/<角色>.vrm` 即可。
